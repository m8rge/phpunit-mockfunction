...
class PHPUnit_Extensions_MockStaticMethod extends PHPUnit_Extensions_MockFunction
{
    ...
    /**
     * Calls original method that we temporary renamed. This maintains the original functionality.
     *
    ...
    {
        /** saving the classname and method name. */
        ...
        if (!class_exists($class_and_method[0])) {
            trigger_error(
                "Class '{$class_and_method[0]}' must exist in order the static method to be mocked.",
                E_USER_ERROR
            );
        }

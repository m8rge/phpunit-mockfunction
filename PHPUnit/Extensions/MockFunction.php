...
class PHPUnit_Extensions_MockFunction
{
    ...
    /**
     * Test case from where the mock function is created. Automatically found with call stack.
     *
    ...
    public function __construct($function_name, $scope_object)
    {
        ...
        $this->mock_object = $this->test_case->getMockBuilder(
    ...
    {
        // Original function is called when the invocation is outsides he scope or
    ...
    /**
     * Calls original function that we temporary renamed. This maintains the original functionality.
     *
     ...
     *
     * Also called method() so after this the mock object can be used to set
    ...
    /**
     * Finds the test object in the call stack that is instance of a PHPUnit test case.
     *
...
CALLBACK;
     ...
     *
     * In theory we should increment the distance by one because when we call this
     * method, we don't count it itself to the call stack, but since the stack is
     * 0-indexed, we can avoid this step.
     *
     * Function calls are ignored, the first call after $distance that is made form
     * is returned.
     *
     * @param int $distance The distance in the call stack from the current call and the desired one.
     * @return object
     */
    protected static function getCallStackObject($distance)
    {
        ...
        do {
            ...
            /* If there is no object associated to this call, we go further until
             * the next one.
             * Function calls and functions like "user_call_func" get ignored.
             */
            ++$distance;
